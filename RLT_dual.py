...
# outer product but now 'b' must be a NUMPY array of constants
def outer(a, b):
    # a: Gurobi MVar or LinExpr vector, b: numpy array
...
def build_dual(data, variant="E1", solve=False, verbose=False):
    ...
    #  for X=X.T and U=U.T
    # OmegaX = mdl.addMVar((n, n),lb=-GRB.INFINITY,ub= GRB.INFINITY,name="OmegaX")
    # OmegaU = mdl.addMVar((n, n),lb=-GRB.INFINITY,ub= GRB.INFINITY,name="OmegaU")
    ...
    # C_x == 0
    Cx = (
      gp.quicksum(alpha[i] * qi[i] for i in range(m))
    + gp.quicksum(beta[j]  * pi[j] for j in range(k))
    + gamma - delta - rho * varphi
    + q0                      # ← constant comes last
    )
    if ell:
        Cx += A.T @ mu + 2 * (A.T @ Theta @ b) + (GAU.T - GAL.T) @ b
    ...
    # C_X == 0
    CX = 0.5 *(Q0 + gp.quicksum(alpha[i] * Qi[i] for i in range(m)) + gp.quicksum(beta [j] * Pi[j] for j in range(k))) - GUU - GLL + GUL 
    if ell:
        CX += - A.T @ Theta @ A - A.T @ (GAU - GAL)
    if eta:
        CX += H.T @ Phi
    mdl.addConstr(CX == 0, name="CX")
    # transpose ekle
